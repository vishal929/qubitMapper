...
map<int,set<int>> addGateEdge(GateNode* chosen, map<int,set<int>>circuitEdges);
 pair<queue<pair<int,int>>,map<int,set<int>>> pruneArchitectureEdges(map<int, set<int>>architectureEdges, int circuitMinDegree);
...
map<int, int> perfectMapper(map<int, set<int>> architectureEdges, set<GateNode*> startSet) {
	...
	while (startGates.size() > 0) {
		...
		int circuitMinDegree=circuitEdges.size();
		int circuitMaxDegree=0;
		...
		int archMinDegree=copyArchitectureEdges.size();
	...
	while (mappingStack.size() > 0) {
		...
		int gateReached = get<2>(stackEntry);
		if (gateReached == mappingQueue.size() - 1) {
			//then we got a perfect initial mapping
			return currMapping;
		}

		GateNode* nextGate = mappingQueue[gateReached + 1];

		if (nextGate->control == -1) {
			//single qubit gate
			//we can just increment counter and skip
			mappingStack.push(make_pair(currMapping, gateReached + 1));
			continue;
		}
		...
		if (currMapping.find(toSatisfy->control) != currMapping.end() && currMapping.find(toSatisfy->target) != currMapping.end()) {
				...
				for (auto i= edges.begin();i!=edges.end();i++){
					if (copyArchitectureEdges[*i].size()>= circuitEdges[unmappedQubit].size() && mappedArchQubits.find(*i)==mappedArchQubits.end()){
						...
						maximalMappingQueue.push(make_tuple(newMapping,archMapping, gateReached+1));
					}
				}
			}
			else {
					...
					//possible pairing for the control, we need to also go through possible pairings for the target
						...
						for (auto j=edges.begin();j!=edges.end();j++){
							if (mappedArchQubits.find(*j)==mappedArchQubits.end() && (copyArchitectureEdges[*j]).size()>=(circuitEdges[toSatisfy->target]).size()  ){
								...
								maximalMappingQueue.push(make_tuple(newMapping, newArchMapping, gateReached+1));
	...
	//firstly pushing all gates to queue and addding them to closed set	
	 ...
	 while (remainingDependencies.size() > 0) {
		 ...
		 if (mapping.find(toAppraise->control) != mapping.end() && mapping.find(toAppraise->target) != mapping.end()) {
			 ...
			 bool allMapped = true;
			 for (auto i = architectureEdges[toAppraise->control];i != architectureEdges[toAppraise->control].end();i++) {
			 ...
			 bool allMapped = true;
			 for (auto i = architectureEdges[toAppraise->target];i != architectureEdges[toAppraise->target].end();i++) {
				 if (mapping.find(*i) == mapping.end()) {
					//then some neighbors is unmapped
					 allMapped = false;
					 break;
...
//returns only a single pathway for swaps to take instead of all of them
 ...
 void singleEntryStitchMappings(pair<map<int, int>, set<int>> mapOne, pair<map<int, int>, set<int>> mapTwo, multimap<int, int> architectureEdges) {
 ...
 pair<map<int,int>,vector<pair<int,int>>> stitchMappings(pair<map<int,int>,set<int>> mapOne, pair<map<int,int>,set<int>> mapTwo, multimap<int,int> architectureEdges) {
	//initializing a priority queue
	 ...
	 swapQueue.push(make_tuple(partialMappingStitchingCost(firstMap, mapTwo), vector<pair<int, int>>, firstMap, firstMap));
	 while (swapQueue.size() > 0) {
		//getting the lowest cost path from the queue
		 tuple<int,
			 vector<pair<int, int>>,
			 map<int, int>,
			 map<int, int>> > result = swapQueue.top();
		 swapQueue.pop();
		 ...
		 for (auto j = transformedMapping.begin();j != transformedMapping.end();j++) {
			 ...
			 if (transformedMapping[j->first] != secondMap[j->first]) {
					 ...
					 if (nodePresent) {
						...
						//we modify our swaps, get the transformed mapping, update the cost and push back to our priority queue
						 ...
						 if (newSwaps.size() > 0) {
							...
							if ((lastSwapTaken.first == associatedLogical && lastSwapTaken.second == *j) || (lastSwapTaken.first == *j && lastSwapTaken.second == associatedLogical)) {
						...
						//then the considered swap has a node that is not considered in the current mapping
						 ...
						 for (auto b = secondMap.begin();b != secondMap.end();b++) {
							 if (transformedMapping.find(b->first) == transformedMapping.end()) {
								 ...
								 //updating cost
								 int newCosts = newSwaps.size() + partialMappingStitchingCost(newTransformedMapping, secondMap, distanceMatrix, architectureEdges);
				 ...
				 if (associatedLogicalQubit != -1) {
					 ...
					 for (auto z = architectureEdges[transformedMapping[associatedLogicalQubit]].begin();z != architectureEdges.end();z++) {	
						 int receivingSwapQubit = -1;
						 for (auto c = transformedMapping.begin();c != transformedMapping.end();c++) {
							if (c->second == z->second) {
						 ...
						 if (receivingSwapQubit==-1) {
							 ...
							 for (auto b = secondMap.begin();b != secondMap.end();b++) {
								 if (transformedMapping.find(b->first) == transformedMapping.end()) {
									 ...
									 //we have to modify our initial mapping to account for this
									 map<int, int> newInitialMapping = initialMapping;
									 newInitialMapping[b->first] = z->second;
									 //updating swaps
									 vector<pair<int, int>>  newSwaps = swapsTaken;
									...
									//then we modify our initial mapping further if possible
									 for (auto d = secondMap.begin();d != secondMap.end();d++) {
										 if (newTransformedMapping.find(d->first) == newTransformedMapping.end()) {
											 ...
											 newInitialMapping[d->first] = c->second;
											
											 //considering the swap
											 vector<pair<int, int>> newSwaps = swapsTaken;
											 newSwaps.push_back(make_pair(d->first, z->first));

											 newTransformedMapping[z->first] = c->second;
 ...
 //this is the amalgamation of all the parts of my project, which will build a circuit for the user and return the circuit
	//if user wants lazy swap insert, then we will just tack all swaps to the end of a maximal partition, in parallel order
		//else, then user wants to try and interweave swaps with circuit execution
 set<GateNode*> circuitBuilder(map<int,set<int>> architectureEdges, set<GateNode*> startGates, bool lazySwapInsert,bool optimal) {
 ...
 void lazySwapCircuitBuilder(map<int,set<int>> architectureEdges, set<GateNode*> startGates,vector<vector<int>> distanceMatrix) {
	 ...
	 pair<map<int, int>,set<int>> lastMapping = NULL;
	 while (nextGates.size() > 0) {
		//we start by getting a maximal mapping
		 ...
		 tuple<map<int, int>, set<int>, set<GateNode*>> > bestTuple = NULL;
		 if (lastMapping == NULL) {
			 int currTupleCost = -1;
			 ...
			 for (int z = 0;z != closenessValues.size();z++) {
